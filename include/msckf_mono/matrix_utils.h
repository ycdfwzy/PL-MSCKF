...
namespace msckf_mono {
  ...
  inline Vector3<_Scalar> getTranslation(const Isometry3<_Scalar>& T) {
    return T.matrix().template block<3, 1>(0, 3);
  }
  ...
  template <typename _Scalar>
    inline Matrix3<_Scalar> Exp(const Vector3<_Scalar>& Theta) {
      Matrix3<_Scalar> skew = vectorToSkewSymmetric(Theta);
      Matrix3<_Scalar> res = Matrix3<_Scalar>::Zero();
      Matrix3<_Scalar> e = Matrix3<_Scalar>::Identity();
      _Scalar t = 1;
      int n = 1;
      _Scalar eps = 1e-9;
      while (e.norm() > eps) {
